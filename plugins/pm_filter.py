...
async def next_page(bot, query):
    ...
    if settings['button']:
        btn = []
        segs = [f"[🔖{get_size(file.file_size)} --- {file.file_name}\n({file.caption})](https://t.me/anime_data_bot?start=file_{file.file_id})" for file in files]
        segs1 = "\n\n".join(segs)
    else:
        btn = []
    try:
